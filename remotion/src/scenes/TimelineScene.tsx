...
/**
 * TimelineScene — Horizontal timeline slide.
 * SVG line drawn left-to-right with node circles popping in sequentially.
 */
  ...
  return (
    ...
    >
      <div style={{ textAlign: 'center', width: '100%' }}>
        ...
        {/* Timeline SVG */}
        <svg width="1920" height={400} style={{ overflow: 'visible' }}>
          {/* Main line */}
          <line
            x1={startX}
            y1={lineY}
          ...
          {items.map((item, i) => {
            ...
            const nodeDelay = 0.4 + i * 0.25;
            const nodeProgress = Math.min(Math.max((t - nodeDelay) / 0.4, 0), 1);
            const nodeScale = interpolate(nodeProgress, [0, 1], [0, 1]);
            const nodeOpacity = interpolate(nodeProgress, [0, 0.5], [0, 1]);
            const isActive = i === Math.min(Math.max(Math.floor((t - 0.4) / 0.25), 0), items.length - 1);
