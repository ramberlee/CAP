...
**抖音**:
- 标题: 15-20字，带话题标签
- 脚本: 约15秒口播文案（50-80字），三段式结构：钩子→价值→收尾，与视频同步
- 视频: 1个约15秒文生视频（Wan2.7-T2V 模型生成），三段式结构：钩子→价值盛宴→情感链接
...
- **视频模型**: wan2.7-t2v-turbo（DashScope，可配置，支持 wan2.7-t2v-plus）
- **视频分辨率**: 1280x720（可配置）
- **视频时长**: 15秒（可配置）
- **处理流程**: 提取口播文字 → TTS 生成音频(.mp3) → 上传音频到 DashScope OSS → 文生视频模型接收 prompt + audio_url → 生成带配音视频 → 保存到 `output/{platform}/media/`
...
(AI技术·术)       │ (热点采集) │     │ (AI内容生成)  │     │ (多平台发布)  │
            ...
            (category: dao/shu)  + media/*.png/*.mp4(配音)
                                       │
                                       ▼
                                  TTS(mimo-v2.5-tts) → OSS上传 → VideoSynthesis(prompt+audio)
```
...
mimo:               # MiMo API 配置
  ...
  base_url:         # API 地址
  model:            # 模型名称
  tts_model:        # TTS 语音合成模型 (mimo-v2.5-tts)
  tts_voice:        # TTS 音色
...
| 阿里云百炼 | AI 配图生成 + 视频生成 | `dashscope.api_key` |
