...
**每个 segment 是一个完整的观点/画面单元**，**不是**一个分句。一个 segment 通常包含 1-3 个短句，朗读时长约 6-12 秒。每个 segment 之后会出现一次画面切换。

- ❌ 错误（过细）：把每句话都拆成独立 segment → 30+ 段，画面切得太碎
- ❌ 错误（过粗）：一个 segment 跨越多个观点，朗读时长 > 15 秒 → 画面跟不上节奏
- ✅ 正确：**一个 segment = 一个观点/一个画面**，朗读 6-12 秒
...
如果你算出来超过 20 段，重新合并 —— 把"承接同一观点"的小句合到一段里。
...
{
  "narration": "GPT-5 来了！OpenAI 刚刚发布最新模型，推理能力提升了整整十倍。而且速度更快，成本更低。这将彻底改变行业格局。关注我，第一时间了解 AI 前沿动态！",
  "segments": [
...
注意示例里只有 4 段（不是 6 段、不是 8 段）—— 每段都是完整观点。
"""
