"""TTS (Text-to-Speech) module — delegates to the appropriate SpeechProvider adapter.

Preserves the TTSSynthesizer class API for existing consumers.
"""
...
class TTSSynthesizer:
    """TTS facade — routes to the configured speech provider.

    Provider is selected by generation.text_provider in config:
      "mimo" (default) or "ark".
    ...
    def split_audio(audio_path: str, output_dir: str, max_segment_duration: float, filename_prefix: str = "seg") -> list[dict]:
        ...
        if total_dur <= max_segment_duration:
            return [{"path": audio_path, "start": 0.0, "end": total_dur, "duration": total_dur}]

        silence_points = []
        try:
            result = subprocess.run(
                [ffmpeg, "-i", audio_path, "-af", "silencedetect=noise=-30dB:d=0.3",
                 "-f", "null", "-"],
        ...
        for i, (start, end) in enumerate(segments):
            ...
            try:
                subprocess.run(
                    [ffmpeg, "-y", "-i", audio_path, "-ss", f"{start:.2f}",
                     "-t", f"{seg_dur:.2f}", "-c", "copy", seg_path],
                    check=True, capture_output=True, text=True, timeout=15,
                )
                result_segments.append({
                    "path": seg_path, "start": round(start, 2),
                    "end": round(end, 2), "duration": round(seg_dur, 2),
                })
            except Exception as e:
    ...
    def synthesize(self, script: str, filename: str, max_duration: float | None = 28,
                   voice_prompt: str | None = None) -> tuple[str, float, float] | None:
        """Generate audio from script text. Returns (file_path, duration, original_duration) or None."""
        ...
        logger.info(f"Generating TTS ({self.text_provider}): {text[:60]}...")
        result = self._provider.synthesize(text, filename, response_format="wav")
        if not result:
            return None
        filepath = result
        ...
        if max_duration and duration > max_duration:
            ...
            self.trim_audio(filepath, max_duration)
            duration = self.get_audio_duration(filepath)
