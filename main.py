...
def get_store(config: AppConfig) -> ContentStore:
    return ContentStore(
...
def generate(
    ...
    if limit is None:
        limit = config.generation.default_limit
...
def publish(
    ...
    d = PublishDispatcher(config, store=store)
...
def run(
    ...
    if gen_summary["contents_created"] > 0:
        ...
        d = PublishDispatcher(config, store=get_store(config))
